...
function CredentialSignIn() {
  ...
  const login = trpc.auth.login.useMutation({
    onSuccess: async (data: any) => {
      if (data?.token) {
        try {
          localStorage.setItem("hms-auth-token", data.token);
          sessionStorage.setItem("hms-auth-token", data.token);
        } catch {}
      }
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (loading) {
    return (
      <div className="grid min-h-screen place-items-center bg-[#f7f7f3]">
        <div className="flex items-center gap-3 rounded-2xl border border-[#e2e8e2] bg-white px-5 py-4 text-sm font-bold text-[#526576]">
  ...
  if (!user) {
    return (
      <main className="paper-noise grid min-h-screen place-items-center bg-[#f7f7f3] px-5 py-10">
        <section className="relative w-full max-w-xl overflow-hidden rounded-[22px] border border-[#d7e8e5] border-t-4 border-t-[#29d0d7] bg-white p-8 shadow-[0_24px_70px_rgba(16,40,58,.12)] md:p-11">
          ...
          <p className="mt-7 text-[10px] font-extrabold uppercase tracking-[.17em] text-[#007c83]">
            Protected clinical workspace
          </p>
          <h1 className="mt-3 font-display text-[42px] leading-[.98] text-[#10283a]">
